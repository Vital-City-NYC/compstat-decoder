              ...
              return (
                <tr key={item.name} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className={`py-1 pl-8 text-right tabular-nums text-sm hidden sm:table-cell ${isVolatile ? 'opacity-50' : ''}`}>
                    <span className={`font-bold ${isVolatile ? 'text-gray-400' : item.diff > 0 ? 'text-orange-700' : item.diff < 0 ? 'text-green-700' : 'text-gray-500'}`}>{signedCount(item.diff)}</span>
                    <span className="text-gray-400 font-normal text-[12px]"> (from {item.prior.toLocaleString()} in {colPrior})</span>
                  </td>
                  <td className={`py-1 pl-3 sm:pl-8 text-right text-xs font-bold tabular-nums whitespace-nowrap ${isVolatile ? 'text-gray-400 opacity-70' : item.pct > 0 ? 'text-orange-600' : item.pct < 0 ? 'text-green-600' : 'text-gray-500'}`}>
                    {dirPct(item.pct)}
                  </td>
                  {isCitywide && (
